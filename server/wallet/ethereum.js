...
module.exports = {
    ...
    pay: async _swap => {
        ...
        const input = `pragma solidity ^0.4.0; contract HTLC { uint public lockTime = ${
            _swap.timeLock2
        } seconds; address public toAddress = ${_swap.buyerAddress2}; bytes32 public hash = 0x${
            _swap.hash
        }; uint public startTime = now; address public fromAddress; string public key; uint public fromValue; function HTLC() payable { fromAddress = msg.sender; fromValue = msg.value; } modifier condition(bool _condition) { require(_condition); _; } function checkKey(string _key) payable condition ( sha256(_key) == hash ) returns (string) { toAddress.transfer(fromValue); key = _key; return key; } function withdraw () payable condition ( startTime + lockTime < now ) returns (uint) { fromAddress.transfer(fromValue); return fromValue; } }`
    ...
    spend: async _swap => {
        ...
        const input = `pragma solidity ^0.4.0; contract HTLC { uint public lockTime = ${seconds} seconds; address public toAddress = ${
            _swap.buyerAddress1
...
async function getContractAddress(addressHash) {
    return new Promise((resolve, reject) => {
        request(
            `https://api-rinkeby.etherscan.io/api?module=proxy&action=eth_getTransactionReceipt&txhash=${addressHash}`,
            (err, res, body) => {
                try {
